...
impl<N: Network> BFT<N> {
    ...
    async fn commit_leader_certificate<const ALLOW_LEDGER_ACCESS: bool, const IS_SYNCING: bool>(
        ...
        let mut leader_certificates = vec![root_leader_certificate.clone()];
        {
            ...
            for round in
                (self.dag.read().last_committed_round() + 2..=root_leader_round.saturating_sub(2)).rev().step_by(2)
            {
                ...
                let Some(previous_leader_certificate) =
                    self.dag.read().get_certificate_for_round_with_author(round, previous_leader)
                else {
                    continue;
                };
