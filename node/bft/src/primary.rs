...
impl<N: Network> Primary<N> {
    ...
    async fn check_proposed_batch_for_expiration(&self) -> Result<()> {
        ...
        let is_expired = match self.proposed_batch.read().as_ref() {
            Some(proposal) => {
                println!("proposal.round(): {}", proposal.round());
                println!("self.current_round(): {}", self.current_round());
    ...
    async fn sync_with_batch_header_from_peer(
        ...
        let Ok(committee) = self.ledger.get_committee_for_round(batch_round) else {
            bail!("Cannot propose a batch for round {batch_round}: the committee is not known yet")
        };
